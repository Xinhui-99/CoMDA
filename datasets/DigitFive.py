...
class DigitFiveDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        img, label = self.data[index], self.data_labels[index]
        if img.shape[0] != 1:
...
def get_digit_five_split_sampler(labels):
    """
    ...
    loc = len(labels)
    loc = loc.view(-1)
    inds = np.random.permutation(loc.size(0))
    # do random perm to make sure uniform sample
    sampler_train.extend(loc[inds].tolist())
...
def digit5_dataset_read(base_path, domain, batch_size):
    ...
    transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize((0.44, 0.44, 0.44), (0.19, 0.19, 0.19))
    ])
    ...
    #sampler_train = get_digit_five_split_sampler(torch.LongTensor(data_labels))

    # raise train and test data loader
    train_dataset = DigitFiveDataset(data=train_image, data_labels=train_label, transform=transform)
    train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, num_workers=4)
    test_dataset = DigitFiveDataset(data=test_image, data_labels=test_label, transform=transform_test)
