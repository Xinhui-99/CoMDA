...
def read_office_home_data(dataset_path, domain_name):
    data_paths = []
    data_labels = []
...
def get_office_home_dloader(base_path, domain_name, batch_size, num_workers):

    dataset_path = path.join(base_path, 'dataset', 'OfficeHome')
    data_paths, data_labels = read_office_home_data(dataset_path, domain_name)
    ...
    train_dataset = OfficeHome(data_paths, data_labels, transforms_train, domain_name)
    test_dataset = OfficeHome(data_paths, data_labels, transforms_test, domain_name)

    sampler_train, sampler_test = get_office_home_split_sampler(torch.LongTensor(data_labels))
    train_dloader = DataLoader(train_dataset, batch_size=batch_size, num_workers=num_workers,
                               pin_memory=True, sampler=sampler_train)
    test_dloader = DataLoader(test_dataset, batch_size=batch_size, num_workers=num_workers,
